...
@Component({
  selector: 'app-historia-peru',
  templateUrl: './historia-peru.component.html',
  styleUrls: ['./historia-peru.component.css']
})

export class HistoriaPeruComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('Prácticas | His. Perú');
  }
  ...
  init_data() {
    ...
    this.proceder = false;
    this.iniciando = true;
    this._preguntaService.obtener_preguntas_practica_historia_peru(this.token).subscribe(
      response => {
  ...
  calificar() {
    ...
    this._userService.registro_resultado_practicas_historia_peru(this.resultado, this.token).subscribe(
      response => {
      }
    );
  }
  ...
  public descargarPDF(): void {
    ...
    html2canvas(element!).then((canvas) => {
      ...
      pdf.save('solucionario-practica-hp.pdf');
