...
export class FisicaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('Prácticas | Física');
  }
  ...
  init_data() {
    ...
    this.proceder = false;
    this.iniciando = true;
    this._preguntaService.obtener_preguntas_practica_fisica(this.token).subscribe(
      response => {
  ...
  calificar() {
    ...
    this._userService.registro_resultado_practicas_fisica(this.resultado, this.token).subscribe(
      response => {
      }
    );
  }
  ...
  public descargarPDF(): void {
    ...
    html2canvas(element!).then((canvas) => {
      ...
      pdf.save('solucionario-practica-fisica.pdf');
