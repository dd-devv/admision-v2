...
@Component({
  selector: 'app-historia-universal',
  templateUrl: './historia-universal.component.html',
  styleUrls: ['./historia-universal.component.css']
})

export class HistoriaUniversalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('Prácticas | His. Universal');
  }
  ...
  init_data() {
    ...
    this.proceder = false;
    this.iniciando = true;
    this._preguntaService.obtener_preguntas_practica_historia_universal(this.token).subscribe(
      response => {
  ...
  calificar() {
    ...
    this._userService.registro_resultado_practicas_historia_universal(this.resultado, this.token).subscribe(
      response => {
      }
    );
  }
  ...
  public descargarPDF(): void {
    ...
    html2canvas(element!).then((canvas) => {
      ...
      pdf.save('solucionario-practica-hu.pdf');
