...
@Component({
  ...
  styleUrls: ['./anatomia.component.css']
})
...
export class AnatomiaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('Prácticas | Anatomía');
  }
  ...
  init_data() {
    ...
    this.proceder = false;
    this.iniciando = true;
    this._preguntaService.obtener_preguntas_practica_anatomia(this.token).subscribe(
      response => {
  ...
  calificar() {
    ...
    this._userService.registro_resultado_practicas_anatomia(this.resultado, this.token).subscribe(
      response => {
      }
    );
  }
  ...
  public descargarPDF(): void {
    ...
    html2canvas(element!).then((canvas) => {
      ...
      pdf.save('solucionario-practica-anatomia.pdf');
