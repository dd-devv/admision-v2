...
@Component({
  selector: 'app-raz-verbal',
  templateUrl: './raz-verbal.component.html',
  styleUrls: ['./raz-verbal.component.css']
})


export class RazVerbalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('Prácticas | Raz. Verbal');
  }
  ...
  init_data() {
    ...
    this.proceder = false;
    this.iniciando = true;
    this._preguntaService.obtener_preguntas_practica_raz_verbal(this.token).subscribe(
      response => {
  ...
  calificar() {
    ...
    this._userService.registro_resultado_practicas_raz_verbal(this.resultado, this.token).subscribe(
      response => {
      }
    );
  }
  ...
  public descargarPDF(): void {
    ...
    html2canvas(element!).then((canvas) => {
      ...
      pdf.save('solucionario-practica-raz-verbal.pdf');
