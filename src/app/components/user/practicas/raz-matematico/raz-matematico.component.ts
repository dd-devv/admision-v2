...
@Component({
  selector: 'app-raz-matematico',
  templateUrl: './raz-matematico.component.html',
  styleUrls: ['./raz-matematico.component.css']
})


export class RazMatematicoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('Prácticas | Raz. Matemático');
  }
  ...
  init_data() {
    ...
    this.proceder = false;
    this.iniciando = true;
    this._preguntaService.obtener_preguntas_practica_raz_matematico(this.token).subscribe(
      response => {
  ...
  calificar() {
    ...
    this._userService.registro_resultado_practicas_raz_matematico(this.resultado, this.token).subscribe(
      response => {
      }
    );
  }
  ...
  public descargarPDF(): void {
    ...
    html2canvas(element!).then((canvas) => {
      ...
      pdf.save('solucionario-practica-raz-matematico.pdf');
