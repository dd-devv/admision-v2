...
export class EditCuentasComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._title.setTitle('ADMIN | Aditar Cuenta');
    ...
    this._route.params.subscribe(
      ...
      params => {
        ...
        this._userService.obtener_cuenta_admin(this.id, this.token).subscribe(
          response => {
  ...
  actualizar(actualizarForm: any){
    ...
    if(actualizarForm.valid){
      this.load_btn = true;
      this._userService.actualizar_cuenta_admin(this.id, this.cuenta, this.token).subscribe(
        response => {
          if (response.data == undefined) {
            ...
            this._router.navigate(['/admin/cuentas']);
          }
        }
      );
    }
